-- Store the recipient's name as supplied by the asker's contacts, so the
-- reveal/answer/receipts screens can identify them even when they don't have
-- an app account. users.display_name still wins when it's set.
...
BEGIN
  ...
  SELECT q.id, q.deep_link_id, q.asker_id, q.recipient_id, q.answer, q.status,
         ...
         a.display_name,
         COALESCE(r.display_name, q.recipient_name),
         q.receipt_number,
